...
instance Script Scene1 where
    run (Scene1) = [ do
            ...
            return . state $ \s -> flip runState s $ do
                createObject $ buildObjectJSON (TransformComponent Blocked (Mat.unit 4 `Mat.times` buildTranslationMatrix (4,4) [5,0,0])) (CharacterComponent 10 5 10 Betuol [(Betuol, 0)])
                createObject $ buildObjectJSON (TransformComponent Blocked (Mat.unit 4 `Mat.times` buildTranslationMatrix (4,4) [5,0,0])) (CharacterComponent 10 5 10 Dunteg [(Dunteg, 0)])
                return ()
